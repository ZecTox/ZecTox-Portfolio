  ...
  function initGSAPAnimations() {
    ...
    if (typeof SplitType !== 'undefined') {
        ...
        if (heroTitle) {
            ...
            const heroSiblings = document.querySelectorAll('.hero-content > :not(h1)');
...
function initCursor() {
    ...
    if (cursor && ring) {
        ...
        let xToRing = gsap.quickTo(ring, "x", {duration: 0.5, ease: "power3"}),
        ...
        const interactives = document.querySelectorAll('a, button, .project-card, .btn');
...
document.addEventListener("DOMContentLoaded", () => {
    setTimeout(initCursor, 500); // Slight delay to ensure GSAP is ready if loaded async
});

document.addEventListener("lenisAndGsapReady", initGSAPAnimations);
})();
